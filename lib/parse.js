export function normalizeMAC(raw) {
  ...
  // Huawei : xxxx-xxxx-xxxx (4+hyphen+4+hyphen+4)
  const hw = s.match(/^([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{4})$/i);
  if (hw) return (hw[1] + hw[2] + hw[3]).toUpperCase().match(/.{2}/g).join(':');
...
export function detectFormat(text) {
  ...
  if (/MAC\s+Address\s+VLAN\/VSI/i.test(text)) return 'huawei';
  return 'simple';
}

export function parseHuaweiTable(text) {
  const entries = [];
  for (const line of text.split('\n')) {
    if (/MAC\s+Address/i.test(line)) continue;
    if (/^[-\s]+$/.test(line)) continue;
    if (/display\s+mac-address/i.test(line)) continue;
    const m = line.match(
      /^\s*([0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4})\s+(\S+)\s+(\S+)\s+(\S+)/i
    );
    if (!m) continue;
    const vlan = m[2].split('/')[0]; // extrait VLAN de "1/-/-"
    entries.push({ vlan, rawMac: m[1], port: m[3], type: m[4] });
  }
  return entries;
}
...
export function parseHPTable(text) {
  ...
  for (const line of text.split('\n')) {
    ...
    if (/^[-\s]+$/.test(line)) continue;
    const m = line.match(/^\s*([0-9a-f]{6}-[0-9a-f]{6})\s+(\S+)\s+(\S+)/i);
    if (!m) continue;
    entries.push({ rawMac: m[1], port: m[2], vlan: m[3] });
  }
  return entries;
}
